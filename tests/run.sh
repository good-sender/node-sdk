#!/usr/bin/env bash
# Single-language conformance driver for good-sender/node-sdk.
#
# Usage:
...
build_sdk() {
  if [[ ! -d "$SDK_DIR/dist" ]]; then
    (cd "$SDK_DIR" && npm install --no-audit --no-fund && npm run build) >/dev/null
  fi
}

install_runner() {
  if [[ ! -d "$SCRIPT_DIR/node_modules" ]]; then
    (cd "$SCRIPT_DIR" && npm install --no-audit --no-fund --ignore-scripts) >/dev/null
  fi
}
...
case "$MODE" in
  mock)
    ...
    build_sdk
    install_runner
    (cd "$SCRIPT_DIR" && npx tsx runner.ts)
    ;;
  dev)
    ...
    build_sdk
    install_runner
    (cd "$SCRIPT_DIR" && npx tsx conformance.ts)
    ;;
